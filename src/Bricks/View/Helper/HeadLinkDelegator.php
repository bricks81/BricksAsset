...
class HeadLinkDelegator extends ZFHeadLink implements DelegatorFactoryInterface {
	...
	public function createDelegatorWithName(ServiceLocatorInterface $serviceLocator,$name,$requestedName,$callback){
		$headLink = new self();		
		$headLink->setAssetService($serviceLocator->getServiceLocator()->get('Bricks\AssetService'));
		return $headLink;	 		
	}
	...
	public function itemToString(stdClass $item){
		...
		$lessSupport = $this->getAssetService()->getLessSupport($moduleName);
		$scssSupport = $this->getAssetService()->getScssSupport($moduleName);
